...
public class BoardRepController implements MemberSessionName {
	...
	public String addReply(@RequestBody Map<String,Object> map, HttpSession session) {
		...
		dto.setId( (String)session.getAttribute(LOGIN) );	//login되어있는 사람만 답글달 수 있게 추가 설정 필요
		dto.setWrite_group( Integer.parseInt( (String)map.get("write_no")) );	//원글에 대한 번호로 groupId로 설정
	...
	@GetMapping(value="replyData/{write_group}", produces="application/json; cahrset=utf-8")
